/*!
 * @details     int32_t Barrett_mul_on_the_fly_asm(int32_t a, int32_t b);
 * @brief       barrett-based multiplication using on-the-fly technique return a X b mod^{+-} Q
 * @param[in]   signed 32-bit data : a, b q
 * @param[out]  return a X b mod^{+-} Q
 * @note        ..
 * @pre         ..
 */
            NAME    Barrett_mul_on_the_fly_asm  ;   module name
            PUBLIC  Barrett_mul_on_the_fly_asm  ;   make the main label vissible outside this module
...
Barrett_mul_on_the_fly_asm:
    ...
    //  bp_lo = (int32_t)(uint16_t)RmodQhi;  bp_hi = (int32_t)(int16_t)(RmodQhi >> 16);
